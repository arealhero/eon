...
lexer_add_keyword(Lexer* lexer, const Token keyword) // TODO(vlad): Get 'keyword' by pointer?
{
    ...
    if (lexer->keywords_count >= lexer->keywords_capacity)
    {
        ...
        lexer->keywords = (Token*) realloc(lexer->keywords, (usize)(size_of(Token) * lexer->keywords_capacity));
    }
...
lexer_add_identifier(Lexer* lexer, const Token identifier) // TODO(vlad): Get 'identifier' by pointer?
{
    ...
    if (lexer->identifiers_count >= lexer->identifiers_capacity)
    {
        ...
        lexer->identifiers = (Token*) realloc(lexer->identifiers, (usize)(size_of(Token) * lexer->identifiers_capacity));
    }
...
{
    lexer->input = input;
    lexer->input_length = (ssize)strlen(input);
...
lexer_scan(Lexer* lexer, Token* next_token)
{
    ...
    {
        char* identifier = calloc(sizeof(char), (usize)(lexer->input_length - lexer->peek_index - 1));
        ssize index = 0;
